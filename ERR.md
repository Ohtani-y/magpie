...
- フォルダ検出ロジックの改善
- **レガシースクリプトの完全整理**（15個のmagpie-*.shスクリプト）
- **一時ファイル生成問題の解決**（/tmp問題の根本修正）
- **使用説明書の更新**（モデル8追加、引数順序修正）
- **統合インターフェースの完成**（run_math_generation.sh）

### ✅ 追加で解決した問題
- `/tmp`一時ファイル作成問題（環境変数方式に変更）
- bcコマンド未導入問題（apt-get install bc完了）
- テスト用モデル追加（Qwen2.5-3B-Instruct）
- ドキュメント不整合問題（使用説明書完全更新）
